...
export default function RiderPage() {
  ...
  const renderPopupIcon = (type: string) => {
    switch (type) {
      ...
      case 'error': return <div className="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-rose-500/20 mb-4 animate-bounce"><X className="h-10 w-10 text-rose-400" /></div>;
      case 'warning': return <div className="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-amber-500/20 mb-4 animate-bounce"><AlertTriangle className="h-10 w-10 text-amber-400" /></div>;
      default: return <div className="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-blue-500/20 mb-4 animate-bounce"><Info className="h-10 w-10 text-blue-400" /></div>;
    }
  };
  ...
  if (!currentUser) return (
    <div className="h-[100dvh] bg-indigo-950 flex flex-col items-center justify-center text-white overflow-hidden">
      <div className="w-12 h-12 border-4 border-indigo-800 border-t-blue-400 rounded-full animate-spin mb-4"></div>
      <h2 className="font-bold text-sm tracking-wider text-indigo-300 animate-pulse">กำลังเตรียมระบบ...</h2>
    </div>
  );

  if (gpsEnabled === false) {
    return (
      <div className="h-[100dvh] bg-indigo-950 flex flex-col items-center justify-center p-6 text-center relative overflow-hidden text-white" style={{ zIndex: 50 }}>
        <div className="absolute inset-0 flex items-center justify-center opacity-20 pointer-events-none">
          <div className="absolute w-64 h-64 border border-blue-500 rounded-full" style={{ animation: 'ping 3s ease-in-out infinite' }}></div>
        </div>
        <div className="relative z-10 bg-indigo-900/90 backdrop-blur-xl p-6 rounded-3xl border border-indigo-800 shadow-2xl max-w-sm w-full animate-in zoom-in duration-500">
          <div className="w-20 h-20 bg-rose-500/20 border border-rose-500/30 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg" style={{ animation: 'bounce 3s ease-in-out infinite' }}>
            <MapPinned size={32} className="text-rose-400" />
          </div>
          <h1 className="text-xl font-black mb-2 text-white">ระบบต้องการตำแหน่ง</h1>
          <p className="text-indigo-300 font-medium mb-6 text-xs">{locationError || 'กรุณาเปิด GPS เพื่อเข้าใช้งานแอปไรเดอร์'}</p>
          <button onClick={() => window.location.reload()} className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm rounded-xl active:scale-95 transition-all shadow-md shadow-blue-600/30">รีเฟรชหน้าจอ</button>
        </div>
      </div>
    );
  }
    ...
    <div className="h-[100dvh] bg-indigo-950 text-indigo-100 font-sans flex flex-col overflow-hidden transition-colors duration-500">
      ...
      <div className="shrink-0 bg-indigo-950/90 backdrop-blur-xl text-white p-3.5 shadow-md flex justify-between items-center border-b border-indigo-900 z-30">
        <div className="flex items-center">
          <button onClick={() => setIsMenuOpen(true)} className="mr-2 p-1.5 hover:bg-indigo-800 rounded-lg active:scale-90 transition-all cursor-pointer"><Menu size={20} className="text-indigo-300" /></button>
          <div>
            <h1 className="text-base font-black tracking-tight flex items-center drop-shadow-sm text-white"><Zap className="mr-1 text-blue-400 fill-blue-400" size={16} /> RIDER APP</h1>
            <div className="text-[9px] text-indigo-300 mt-0.5 flex items-center font-bold uppercase tracking-wider">
              {myLocation ? <span className="flex items-center text-emerald-400"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400 mr-1 animate-pulse shadow-[0_0_8px_rgba(52,211,153,0.8)]"></div> GPS Active</span> : <span className="opacity-70 animate-pulse text-amber-400">หาสัญญาณ...</span>}
            </div>
          </div>
        </div>
        <div className="bg-indigo-900 border border-indigo-800 px-3 py-1.5 rounded-full text-[10px] font-black tracking-wide text-blue-300 shadow-inner truncate max-w-[100px]">{riderName}</div>
      </div>
        ...
        {activeTab === 'available' && (
          ...
          <div className="flex-1 flex flex-col overflow-hidden w-full pt-4 px-3 sm:px-5 pb-24 animate-in fade-in duration-500">
            ...
            {availableOrders.length === 0 ? (
              <div className="text-center bg-indigo-900/40 rounded-[2rem] border border-indigo-800/50 shadow-sm flex flex-col items-center justify-center mx-auto max-w-xs w-full flex-1 p-6">
                ...
                <p className="text-indigo-200 font-bold mb-1 text-base">ยังไม่มีงานเข้ามา</p>
                <p className="text-xs text-indigo-400 font-medium">รอแอดมินจ่ายงานสักครู่นะครับ ☕</p>
              </div>
            ) : (
          ...
          <div className="flex-1 flex flex-col overflow-hidden w-full pt-4 px-3 sm:px-5 pb-24 animate-in fade-in duration-500">
            ...
            {activeOrders.length === 0 ? (
              <div className="text-center bg-indigo-900/40 rounded-[2rem] border border-indigo-800/50 shadow-sm flex flex-col items-center justify-center mx-auto max-w-xs w-full flex-1 p-6">
                ...
                <p className="text-indigo-200 font-bold mb-1 text-base">ไม่มีงานค้างในมือ</p>
                <p className="text-xs text-indigo-400 font-medium">ไปดูที่ &quot;งานว่าง&quot; เพื่อรับงานต่อ 🛵</p>
              </div>
            ) : (
              <div className="flex-1 overflow-x-auto overflow-y-hidden snap-x snap-mandatory flex gap-3 items-stretch hide-scrollbar -mx-3 px-3 sm:mx-0 sm:px-0">
        ...
        {activeTab === 'history' && (
          <div className="flex-1 overflow-y-auto hide-scrollbar pb-36 pt-4 px-3 sm:px-5 w-full animate-in fade-in duration-500 mx-auto max-w-2xl">
            <h2 className="font-black text-white mb-4 text-lg flex items-center">
              ประวัติรอบวันนี้ 
              <span className="ml-2 px-2 py-0.5 bg-indigo-900 border border-indigo-800 text-indigo-300 text-[10px] rounded-md shadow-sm">{completedOrders.length}</span>
            </h2>
            <div className="space-y-3">
              {completedOrders.length === 0 ? (
                <div className="text-center py-16 bg-indigo-900/40 rounded-3xl border border-indigo-800/50 shadow-sm">
                  ...
                  <p className="text-indigo-300 font-bold text-base">ยังไม่มีงานที่สำเร็จ</p>
                </div>
              ) : completedOrders.map((order, idx) => (
                <div key={order.id} onClick={() => setSelectedViewOrder(order)} className="bg-indigo-900/60 backdrop-blur-sm p-4 rounded-2xl shadow-sm border border-indigo-800 flex justify-between items-center cursor-pointer active:scale-95 transition-all hover:border-blue-500/50 group animate-in slide-in-from-bottom-2" style={{ animationDelay: `${idx * 30}ms`, animationFillMode: 'both' }}>
                  <div className="flex items-center">
                    <div className="w-8 h-8 rounded-full bg-indigo-950 group-hover:bg-blue-500/20 group-hover:text-blue-300 transition-colors text-indigo-400 text-xs font-black flex items-center justify-center mr-3 border border-indigo-800">{completedOrders.length - idx}</div>
                    <div>
                      <div className="font-black text-white text-base group-hover:text-blue-300 transition-colors">{order.order_number}</div>
                      <div className="text-[10px] text-indigo-300 font-medium mt-1 flex items-center">
                        <span className="font-black text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded mr-2 uppercase border border-blue-500/20">{order.job_type}</span>
                        ส่งเมื่อ {order.end_time ? new Date(order.end_time).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' }) : '-'} น.
                      </div>
                    </div>
                  </div>
                  <div className="text-[10px] font-black text-emerald-400 bg-emerald-500/10 px-2.5 py-1.5 rounded-lg flex items-center border border-emerald-500/20"><CheckCircle2 size={12} className="mr-1"/> สำเร็จ</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="fixed bottom-2 left-1/2 transform -translate-x-1/2 w-[92%] max-w-sm z-40">
        <div className="bg-indigo-950/95 backdrop-blur-xl border border-indigo-800 shadow-[0_10px_30px_rgba(0,0,0,0.6)] rounded-2xl p-1 flex items-center justify-between">
          <button onClick={() => setActiveTab('available')} className={`relative flex-1 flex flex-col items-center py-2.5 rounded-xl transition-all cursor-pointer ${activeTab === 'available' ? 'bg-blue-500/20 text-blue-300 shadow-inner border border-blue-500/30' : 'text-indigo-400 hover:text-indigo-200 hover:bg-indigo-900/50'}`}>
            <Zap size={20} className={`mb-1 transition-all ${activeTab === 'available' ? 'fill-blue-400' : ''}`} /><span className="text-[9px] font-black uppercase tracking-widest">งานว่าง</span>
            {availableOrders.length > 0 && <span className="absolute top-1.5 right-1/4 translate-x-2 bg-red-500 text-white text-[9px] font-black rounded-full h-4 min-w-4 px-1 flex items-center justify-center animate-bounce shadow-md">{availableOrders.length}</span>}
          </button>
          <button onClick={() => setActiveTab('jobs')} className={`relative flex-1 flex flex-col items-center py-2.5 rounded-xl transition-all cursor-pointer ${activeTab === 'jobs' ? 'bg-blue-500/20 text-blue-300 shadow-inner border border-blue-500/30' : 'text-indigo-400 hover:text-indigo-200 hover:bg-indigo-900/50'}`}>
            <ClipboardList size={20} className="mb-1" /><span className="text-[9px] font-black uppercase tracking-widest">งานของฉัน</span>
            {activeOrders.length > 0 && <span className="absolute top-1.5 right-1/4 translate-x-2 bg-blue-500 text-white text-[9px] font-black rounded-full h-4 min-w-4 px-1 flex items-center justify-center shadow-md">{activeOrders.length}</span>}
          </button>
          <button onClick={() => setActiveTab('history')} className={`relative flex-1 flex flex-col items-center py-2.5 rounded-xl transition-all cursor-pointer ${activeTab === 'history' ? 'bg-blue-500/20 text-blue-300 shadow-inner border border-blue-500/30' : 'text-indigo-400 hover:text-indigo-200 hover:bg-indigo-900/50'}`}>
            <History size={20} className="mb-1" /><span className="text-[9px] font-black uppercase tracking-widest">ประวัติ</span>
          </button>
        </div>
      </div>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 flex" style={{ zIndex: 110 }}>
          ...
          <div className="relative w-4/5 max-w-xs bg-indigo-950 border-l border-indigo-900 h-full shadow-2xl flex flex-col animate-in slide-in-from-left duration-300 z-10 rounded-r-[2rem] overflow-hidden">
            <div className="bg-gradient-to-br from-indigo-900 to-indigo-950 p-6 text-white relative border-b border-indigo-800">
              <button onClick={() => setIsMenuOpen(false)} className="absolute top-5 right-5 p-1.5 bg-indigo-950 rounded-full hover:bg-indigo-800 transition-all cursor-pointer active:scale-90 border border-indigo-800"><X size={16} /></button>
              <div className="w-14 h-14 bg-indigo-950 rounded-xl flex items-center justify-center mb-4 text-xl font-black shadow-inner border border-indigo-800 text-blue-400">{riderName.charAt(0)}</div>
              <h2 className="font-black text-xl mb-1 tracking-tight text-white">{riderName}</h2>
              <p className="text-indigo-400 text-[10px] flex items-center font-bold tracking-wide"><span className="inline-block w-2 h-2 rounded-full bg-emerald-500 mr-1.5 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></span> พร้อมรับงานเสมอ</p>
            </div>
            <div className="flex-1 p-4 space-y-2 overflow-y-auto">
              <button onClick={() => { setIsMenuOpen(false); setShowDashboard(true); }} className="w-full flex items-center p-3 text-indigo-200 bg-indigo-900/50 hover:bg-indigo-800 hover:text-white rounded-xl transition-all text-sm font-bold cursor-pointer border border-indigo-800/50">
                <div className="w-8 h-8 rounded-lg bg-indigo-950 flex items-center justify-center mr-3"><LayoutDashboard size={16} className="text-blue-400" /></div>
                Dashboard ของฉัน
              </button>
            </div>
            <div className="p-4 border-t border-indigo-900 bg-indigo-950">
              <button onClick={() => { setIsMenuOpen(false); handleLogout(); }} className="w-full flex items-center justify-center p-3 text-rose-400 bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500 hover:text-white rounded-xl transition-all text-sm font-black cursor-pointer shadow-sm active:scale-95"><LogOut size={16} className="mr-2" />ออกจากระบบ</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedViewOrder && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 animate-in fade-in duration-200 backdrop-blur-sm" style={{ zIndex: 200 }}>
          <div className="bg-indigo-950 rounded-[1.5rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-300 border border-indigo-800 flex flex-col" style={{ maxHeight: '85vh' }}>
            <div className="bg-indigo-900 border-b border-indigo-800 p-5 flex justify-between items-center text-white shrink-0">
              <h3 className="font-black flex items-center text-base tracking-tight"><ClipboardList size={18} className="mr-2 text-blue-400"/> รายละเอียดออเดอร์</h3>
              <button onClick={() => setSelectedViewOrder(null)} className="p-1.5 bg-indigo-800 hover:bg-indigo-700 rounded-full transition-all cursor-pointer active:scale-90 hover:rotate-90 duration-300"><X size={16} strokeWidth={3} className="text-indigo-200"/></button>
            </div>
            ...
            <div className="p-5 space-y-5 overflow-y-auto hide-scrollbar">
              <div className="flex justify-between items-end border-b border-indigo-800 pb-4">
                <div>
                  <div className="text-[10px] font-black text-indigo-400 mb-1 tracking-wider uppercase">เลขที่ออเดอร์</div>
                  <div className="text-2xl font-black text-white tracking-tighter">{selectedViewOrder.order_number}</div>
                </div>
              ...
              {selectedViewOrder.menu && (
                <div className="space-y-2">
                  <div className="text-[10px] font-black text-indigo-400 uppercase tracking-wider">รายการที่สั่ง</div>
              ...
              {selectedViewOrder.details && (
                <div className="space-y-2">
                  <div className="text-[10px] font-black text-indigo-400 uppercase tracking-wider">หมายเหตุ (Note)</div>
              ...
              {/* รูปภาพในการดูรายละเอียดจัดเรียงลงมาเต็มใบ */}
              ...
              <div className="bg-indigo-900/50 rounded-xl border border-indigo-800 p-4 space-y-3 text-xs">
                <div className="flex justify-between items-center"><span className="text-indigo-300 font-medium">ประเภทงาน:</span><span className="font-black text-white uppercase px-2.5 py-1 bg-indigo-800 rounded-md shadow-sm">{selectedViewOrder.job_type}</span></div>
                <div className="flex justify-between items-center pt-2.5 border-t border-indigo-800/50"><span className="text-indigo-300 font-medium">ยอดเรียกเก็บ:</span><span className="font-black text-white text-lg">฿{selectedViewOrder.total_price}</span></div>
                <div className="flex justify-between items-center"><span className="text-indigo-300 font-medium">การชำระเงิน:</span><span className={`font-black text-[9px] uppercase px-2 py-1 rounded ${selectedViewOrder.payment_method === 'โอน' ? 'bg-blue-500/20 text-blue-300' : 'bg-emerald-500/20 text-emerald-300'}`}>{selectedViewOrder.payment_method || 'เงินสด'}</span></div>
              </div>

              {selectedViewOrder.address && (
                <div className="space-y-2">
                  <div className="text-[10px] font-black text-indigo-400 uppercase tracking-wider">สถานที่จัดส่ง</div>
                  <div className="flex items-start text-xs text-red-200 bg-red-500/10 p-4 rounded-xl border border-red-500/20 font-bold shadow-inner">
                    <MapIcon size={16} className="mr-2 mt-0.5 text-red-400 shrink-0" />
            ...
            <div className="p-4 pt-0 shrink-0 bg-indigo-950 mt-2">
              <button onClick={() => setSelectedViewOrder(null)} className="w-full py-3.5 bg-indigo-900 text-indigo-200 hover:text-white hover:bg-indigo-800 border border-indigo-800 font-black rounded-xl transition-all cursor-pointer shadow-lg active:scale-95 text-xs uppercase tracking-widest">
                ปิดหน้าต่าง
              </button>
            </div>
          </div>
      ...
      {popup.isOpen && (
        <div className="fixed inset-0 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200" style={{ zIndex: 350 }}>
          <div className="bg-indigo-950 rounded-[2rem] shadow-2xl w-full max-w-sm p-6 text-center animate-in zoom-in-95 slide-in-from-bottom-5 duration-300 border border-indigo-800">
            {renderPopupIcon(popup.icon || 'info')}
            <h3 className="text-lg font-black text-white mb-2 tracking-tight">{popup.title}</h3>
            <p className="text-indigo-300 text-xs mb-6 font-medium leading-relaxed">{popup.message}</p>
            {popup.type === 'alert' ? (
              <button onClick={closePopup} className="w-full py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-xl active:scale-95 transition-all shadow-lg shadow-blue-600/30 cursor-pointer text-sm">ตกลง</button>
            ) : (
              <div className="flex gap-3">
                <button onClick={closePopup} className="flex-[0.8] py-3.5 bg-indigo-900 border border-indigo-800 hover:bg-indigo-800 text-indigo-200 font-black rounded-xl active:scale-95 transition-all cursor-pointer text-sm">ยกเลิก</button>
                <button onClick={popup.onConfirm} className="flex-[1.2] py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-xl active:scale-95 transition-all shadow-lg shadow-blue-600/30 cursor-pointer text-sm">ยืนยัน</button>
              </div>
            )}
          </div>
        </div>
