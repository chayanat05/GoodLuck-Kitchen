...
import { X, ArrowLeft, ZoomIn, ChefHat, Bike, ShieldCheck, Phone, MapPin, CalendarDays, Clock } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8">
          <div className="flex items-center gap-4">
            <Link href="/home" className="p-2 bg-white rounded-full shadow-sm border border-slate-200 cursor-pointer active:scale-95"><ArrowLeft size={20} className="text-slate-600" /></Link>
            <div>
              <h1 className="text-2xl font-black text-slate-800">จัดการใบสมัคร</h1>
              <p className="text-xs font-bold text-slate-500">คัดกรองผู้สมัคร {viewMode === 'rider' ? 'ไรเดอร์' : viewMode === 'kitchen' ? 'แม่ครัว' : 'แอดมิน'}</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row h-[75vh]">
          <div className="w-full md:w-1/3 border-r border-slate-100 flex flex-col bg-slate-50/50">
            {/* Search Input */}
            <div className="p-4"><input className="w-full p-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-blue-500/20" placeholder="ค้นหาชื่อ, เบอร์โทร..." value={search} onChange={e => setSearch(e.target.value)} /></div>
            <div className="flex-1 overflow-y-auto thin-scrollbar">
              {filteredApps.length === 0 ? <div className="text-center py-10 text-slate-400 font-bold text-sm">ไม่พบข้อมูลผู้สมัคร</div> : 
                filteredApps.map(app => (
                <div key={app.id} onClick={() => setSelectedApp(app)} className={`p-4 border-b border-slate-100 cursor-pointer flex items-center gap-3 transition-colors ${selectedApp?.id === app.id ? 'bg-indigo-50/80 border-l-4 border-l-indigo-500' : 'hover:bg-white border-l-4 border-l-transparent'}`}>
                  <div className="relative w-12 h-12 rounded-full overflow-hidden shrink-0 bg-slate-200 shadow-sm border border-white">
                    <Image src={app.fb_profile_image} alt="Profile" fill sizes="48px" className="object-cover"/>
                  </div>
                  <div className="min-w-0 flex-1">
                    <h4 className="text-sm font-black text-slate-800 truncate">{app.full_name}</h4>
                    <p className="text-[10px] text-slate-500 font-bold flex items-center gap-1"><Phone size={10}/> {app.phone_number}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {selectedApp ? (
              <div className="animate-in fade-in">
                <div className="flex flex-col sm:flex-row justify-between items-start mb-8 gap-4 border-b border-slate-100 pb-6">
                  <div className="flex items-center gap-4">
                    <div className="relative group cursor-zoom-in shrink-0" onClick={() => setSelectedFullImage(selectedApp.fb_profile_image)}>
                      <Image 
                    ...
                    <div>
                      <h2 className="text-2xl font-black text-slate-800">{selectedApp.full_name}</h2>
                      <div className="text-sm font-bold text-slate-500 flex flex-wrap items-center gap-y-1 gap-x-4 mt-1.5">
                        <span className="flex items-center gap-1"><Phone size={14} className="text-indigo-500"/> {selectedApp.phone_number}</span>
                        ...
                        <span className="flex items-center gap-1 text-slate-600 bg-slate-100 px-2 py-0.5 rounded-lg border border-slate-200">
                          ...
                          {new Date(selectedApp.created_at).toLocaleDateString('th-TH', { month: 'short', day: 'numeric', year: 'numeric'})}
                          <span className="mx-0.5 text-slate-300">•</span>
                          <Clock size={14} className="text-blue-500"/>
                          {new Date(selectedApp.created_at).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })} น.
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2 w-full sm:w-auto shrink-0">
                    <button onClick={() => updateStatus(selectedApp.id, selectedApp.app_type, 'approved')} className="flex-1 sm:flex-none px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-black shadow-sm active:scale-95 transition-all cursor-pointer">รับเข้าทำงาน</button>
                    <button onClick={() => updateStatus(selectedApp.id, selectedApp.app_type, 'rejected')} className="flex-1 sm:flex-none px-4 py-2.5 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-xs font-black shadow-sm active:scale-95 transition-all cursor-pointer">ปฏิเสธ</button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   ...
                   {selectedApp.app_type === 'admin' && (
                    <>
                      <DetailItem label="อายุ" value={selectedApp.age ? `${selectedApp.age} ปี` : '-'} />
                      <DetailItem label="ที่อยู่ปัจจุบัน / การเดินทาง" value={selectedApp.address_commute || '-'} />
                      <DetailItem label="คณะ / ปีที่เรียน" value={selectedApp.faculty_year || '-'} />
                      <DetailItem label="วันที่สะดวกทำงาน" value={selectedApp.availability || '-'} />
                      <DetailItem label="การพิมพ์แชท / สมาธิ" value={selectedApp.typing_speed_focus || '-'} fullWidth />
                      <DetailItem label="ประสบการณ์แอดมิน" value={selectedApp.prev_admin_exp || '-'} fullWidth />
                      <DetailItem label="แนะนำตัว / ทักษะเพิ่มเติม" value={selectedApp.about_me || '-'} fullWidth />
                    </>
                   )}
                   ...
                   {selectedApp.app_type === 'rider' && (
                    <>
                      ...
                      <DetailItem label="รุ่นรถที่ใช้งาน" value={selectedApp.vehicle_model || '-'} />
                      <DetailItem label="ใบขับขี่ / พ.ร.บ." value={selectedApp.driving_license_status || '-'} />
                      <DetailItem label="ที่อยู่ปัจจุบัน" value={selectedApp.current_address || '-'} fullWidth />
                      <DetailItem label="เวลาว่างทำงาน" value={selectedApp.availability || '-'} />
                      <DetailItem label="ทำงานรอบดึก (19.00 - 04.00)" value={selectedApp.late_night_shift || '-'} />
                      <DetailItem label="การรับแรงกดดัน / งานเร่งด่วน" value={selectedApp.handle_pressure || '-'} />
                      <DetailItem label="ครอบครัวอนุญาต" value={selectedApp.family_approval || '-'} />
                      <DetailItem label="ประสบการณ์ / รู้จักพื้นที่ มมส." value={selectedApp.experience_and_area || '-'} fullWidth />
                      <DetailItem label="เริ่มงานได้เมื่อไหร่ / สัญญา 10 เดือน" value={selectedApp.start_date_and_commitment || '-'} fullWidth />
                    </>
                   )}
                   ...
                   {selectedApp.app_type === 'kitchen' && (
                    <>
                      ...
                      <DetailItem label="การเดินทางมาทำงาน" value={selectedApp.can_commute || '-'} />
                      <DetailItem label="คณะ / ปีที่เรียน" value={selectedApp.education || '-'} />
                      ...
                      <DetailItem label="ครอบครัวอนุญาต" value={selectedApp.family_approval || '-'} />
                      <DetailItem label="การรับแรงกดดัน" value={selectedApp.handle_pressure || '-'} />
                      <DetailItem label="ประสบการณ์การทำอาหาร / ทำงาน" value={selectedApp.experience || '-'} fullWidth />
                      <DetailItem label="แนะนำตัว" value={selectedApp.about_me || '-'} fullWidth />
                    </>
                   )}
                </div>
              </div>
                 ...
                 <p className="font-bold tracking-widest uppercase">เลือกใบสมัครเพื่อดูรายละเอียด</p>
                </div>}
          </div>
        </div>
      </div>
      ...
      {selectedFullImage && (
        <div 
          ...
          onClick={() => { setSelectedFullImage(null); setIsZoomed(false); }}
        >
          <button 
            ...
            onClick={(e) => { e.stopPropagation(); setSelectedFullImage(null); setIsZoomed(false); }}
          >
            <X size={24} />
          </button>
...
function DetailItem({ label, value, fullWidth = false }: { label: string, value: string, fullWidth?: boolean }) {
  return (
    <div className={`bg-slate-50 p-4 rounded-2xl border border-slate-100 ${fullWidth ? 'md:col-span-2' : ''}`}>
      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{label}</p>
      <p className="text-sm font-bold mt-1.5 text-slate-700 whitespace-pre-line leading-relaxed">{value}</p>
    </div>
  );
}